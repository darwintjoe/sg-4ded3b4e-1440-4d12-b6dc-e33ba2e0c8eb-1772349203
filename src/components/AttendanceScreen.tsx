...
export function AttendanceScreen({ onBack }: { onBack: () => void }) {
  ...
  if (showSuccess) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-600 to-emerald-700 p-4 overflow-hidden">
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-900 via-cyan-900 to-teal-900 p-4 overflow-hidden">
      {/* Top Bar */}
      ...
      <div className="w-full max-w-md flex flex-col items-center space-y-5">
        ...
        <div className="flex justify-center">
          <div className="bg-gradient-to-br from-blue-500 to-cyan-600 p-5 rounded-2xl shadow-2xl">
        ...
        <p className="text-base text-cyan-200 font-medium text-center -mt-2">
          {translate("attendance.subtitle", language)}
        </p>

        {/* Mode Selector - Clock In/Out - Compact */}
        <div className="w-full grid grid-cols-2 gap-3">
          <Button
            variant={mode === "clockIn" ? "default" : "outline"}
            onClick={() => setMode("clockIn")}
            className={`h-20 rounded-xl text-sm font-bold transition-all shadow-lg hover:shadow-xl ${
              mode === "clockIn"
                ? "bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 scale-105 text-white"
                : "bg-white/10 backdrop-blur hover:bg-green-500/30 border-white/20 text-white"
            }`}
          >
          ...
          <Button
            ...
            className={`h-20 rounded-xl text-sm font-bold transition-all shadow-lg hover:shadow-xl ${
              mode === "clockOut"
                ...
                : "bg-white/10 backdrop-blur hover:bg-red-500/30 border-white/20 text-white"
            }`}
          >
            <div className="flex flex-col items-center gap-2">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
              <Button
                ...
                className="h-16 text-2xl font-bold rounded-xl bg-white/10 backdrop-blur hover:bg-cyan-500/30 hover:scale-105 transition-all shadow-md hover:shadow-xl border-white/20 text-white"
              >
                {num}
              </Button>
            ))}
            ...
            <Button
              ...
              className="h-16 text-xl font-bold rounded-xl bg-gradient-to-r from-cyan-400 to-blue-500 hover:from-cyan-500 hover:to-blue-600 hover:scale-105 transition-all shadow-lg hover:shadow-xl text-slate-900"
            >
              ✓
            </Button>
          </div>
